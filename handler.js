...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 7) {
                    this.reply(m.chat, `Votre limite a atteint la limite\n\n•••••••••••••••••••\n\nآDésolé, je suis arrivé à l’essentiel. 🙂 

Le bot est programmé pour répondre à *10* demandes par jour, puis il faut attendre 24 heures pour bénéficier d’un *10* supplémentaire Et ainsi de suite... Donc, travaillez toujours avec le bot quand vous en avez besoin juste pour ne pas perdre d’opportunités, \n\n*à demain j’espère*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `.Niveau requis ${plugin.level} Pour utiliser cette commande\n*Votre niveau:* ${_user.level}`, m)
                ...
                try {
                    ...
                    if (!isPrems) {
                        ...
                        if (plugin.limit) return m.reply(`🔖 Limite utilisée: ${m.limit * 1}\nRestant: ${global.db.data.users[m.sender].limit - 1}`)
                        }
                } catch (e) {
                    // Error occured
                    m.error = e
    ...
    this.sendMessage(id, {
        ...
        mentionedJid: [user],
            externalAdReply: {
                containsAutoReply: true,
                mediaType: 1,
                mediaUrl: 'https://telegra.ph/file/74abb87ac82571db546.jpg',
                renderLargerThumbnail: true,
                showAdAttribution: true,
                sourceUrl: null,
                thumbnailUrl: pp,
...
let msg = {
        premium: 'Padon, komand sa a ka sèlman itilize pa itilizatè yo\n *Premium*',
        group: 'baz komnd sa ka use nan group selman man 👀👍',       
        private: 'komand sa a ka itilize sèlman nan prive',       
        botAdmin: 'Metem admin la, kidonk mwen ka jwenn aksè nan gwoup la',
        admin: 'Baz bon w pa menm metem admin gadon kk 😞 ',
        restrict: 'Restriksyon pa dispo sou pou chat sa a',
        adminonly: 'Moun ki admin ki ka ise chawony 👀',
        premiumonly: `📢 لا Ou ka jwenn aksè nan komand ❗ nan \nPremium vèsyon an prim jwenn aksè nan li atravè òdinatè w lanر.\ncontact the owner below\nwa.me/${global.info.nomerown}`,
        groups: 'komand nan kounye a aktif pou gwoup sèlman.',
        gconly: `📢 Ou pa ka jwenn aksè nan \NAntre nan group nou an pou jwenn aksè ak bot la\n\n> ${global.url.sgc}`,
        game: 'komand nan jwèt * pa aktive sou pou chat sa a',
        rpg: 'komand nan * Rp * pa disponib sou pou chat sa a',
        rowner: 'Yon komand espesyal pou mèt bot la reyèl, *itilizatè * pa ka jwenn aksè nan li :!',
        owner: 'komand la se mèt bot la ak * itilizatè * pa ka jwenn aksè nan li:! Oswa itilize li ',
        mods: 'komand sa a se pou administratè sèlman'
        }[type]
  ...
  if (msg) return conn.sendMessage(m.chat, {
    ...
    let daftar = {
  unreg: `\`Vous n’êtes pas encore inscrit dans la base de données. Inscrivez-vous instantanément par réservationة:\`
  
- /daftar name. age\n\n Avant de cliquer sur le bouton de connexion, assurez-vous que vous êtes dans mon groupe WhatsApp \n\n https://whatsapp.com/coming soon`}[type]
  
  if (daftar) return conn.sendUrlImageButton(m.chat, daftar, [{name: "quick_reply", buttonParamsJson: `{"display_text": "idantifyew 💳", "id": "@verify"}`}], wm, registrasi, fkon)
        }
